...
namespace FinalProject
{
    public partial class ViewPatient : Form
    {
        ...
        private void PatientsDGV_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
            ...
            PAddressTb.Text = PatientsDGV.SelectedRows[0].Cells[4].Value.ToString();
            PGenderTb.SelectedItem = PatientsDGV.SelectedRows[0].Cells[5].Value.ToString();
            PBloodTb.SelectedItem = PatientsDGV.SelectedRows[0].Cells[6].Value.ToString();
            if (PNameTb.Text == "")
            {
                key = 0;
            }
        ...
        {
            if (PNameTb.Text == "" || PPhoneTb.Text == "" || PAgeTb.Text == "" || PAddressTb.Text == "" || PGenderTb.SelectedIndex == -1 || PBloodTb.SelectedIndex == -1)
            {
            ...
            else
            {
                try
                {
                    string query = "update PatientTbl set PName='"+PNameTb+"',Page="+PAgeTb+",PPhone='"+PPhoneTb.Text+"',PGender='"+PGenderTb.SelectedItem.ToString()+"',PBlood='"+PBloodTb.SelectedItem.ToString()+"',PAddress='"+PAddressTb.Text +"' where PNum = " + key + ";";
                    Con.Open();
                    SqlCommand cmd = new SqlCommand(query, Con);
                    cmd.ExecuteNonQuery();
                    MessageBox.Show("Patient Successfully Deleted");
